...
bool create_server_listener(const char* ip, short port)
{
    ...
    memset(&e, 0, sizeof(e));
    //EPOLLRDHUP:如果对端关闭，会发送一个可读事件过劳，如果本端读了，会报错。所以新增EPOLLRDHUP，就在底层处理了
...
void* worker_thread_func(void* arg)
{
    while (!g_bstop) {
        ...
        //gdb调试时不能实时刷新标准输出，用这个函数刷新标准输出，使信息在屏幕上试试显示出来
        std::cout << std::endl;
        ...
        std::ostringstream ostreamstr;
        ostreamstr << "[" << nowstr->tm_year + 1900 << "-"
        ...
        strclientmsg.insert(0, ostreamstr.str());
...
void daemon_run()
{
    ...
    //之前parent和child运行在同一个session中国，parent是会话（session）的领头进程
    //parent进程作为会话的领头进程，如果exit结束执行的话，那么子进程会成为孤儿进程，并被init进程收养
    //执行setsid()之后，child将重新获得一个新的会话ID
    //这时parent退出之后，将不会影响child了
    setsid();
